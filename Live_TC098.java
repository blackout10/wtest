 ...
 * Subject : CJ Mall 운영 
 * Name : TC_098
 * Scenario : ID/Password 오입력시, 얼럿 노출 확인
 * Assertion : "아이디 또는 비밀번호가 정확하지 않습니다. 다시 확인하여 입력해주시기 바랍니다." 메시지 확인
 *
 */

public class Live_TC098 {
	...
	@Test
	public void TC098() throws Exception {
		try{
            ...
            // 로그인 시도 - 아이디, 잘못된 패스워드 입력
		    driver.findElement(By.xpath(".//*[@id='mid_1']")).click();
		    driver.findElement(By.xpath(".//*[@id='mid_1']")).clear();
		    driver.findElement(By.xpath(".//*[@id='mid_1']")).sendKeys(word_tc098_01);
		    driver.findElement(By.xpath(".//*[@id='mpass']")).clear();
		    driver.findElement(By.xpath(".//*[@id='mpass']")).sendKeys(word_tc098_02);
		    driver.findElement(By.xpath(".//*[@id='login_area']/form/div/div/div[2]/input")).click();
			System.out.println("로그인 시도 - 아이디, 잘못된 패스워드 입력");
		    
		    // "아이디 또는 비밀번호가 정확하지 않습니다. 다시 확인하여 입력해주시기 바랍니다." 얼럿 메시지 확인
		    assertTrue(closeAlertAndGetItsText().matches("^아이디 또는 비밀번호가 정확하지 않습니다\\.\n다시 확인하여 입력해주시기 바랍니다."));
		    if(acceptNextAlert){
		    	System.out.println("[TC_098] success");
				assertTrue(true);
				return;
		    } else {
		    	System.out.println("[TC_098] failure : '아이디 또는 비밀번호가 정확하지 않습니다. 다시 확인하여 입력해주시기 바랍니다.' 얼럿 메시지 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_098] failure");
			assertTrue(false);
		}
	}	
